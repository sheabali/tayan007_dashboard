    ...
    <div className="bg-white p-6 rounded-2xl border border-slate-100 shadow-sm flex flex-col h-full w-full">
      ...
      <div className="flex flex-col flex-1">
        {activities.map((activity, idx) => (
          <div key={idx}>
            <div className="flex items-start gap-4 group transition-transform duration-200 hover:translate-x-1 py-3">
              <div
                className={`w-9 h-9 rounded-full ${activity.avatarBg} ${activity.avatarColor} flex items-center justify-center shrink-0 border border-amber-100/50 shadow-sm`}
              >
            ...
            {idx !== activities.length - 1 && <Separator className="my-2" />}
          </div>
        ))}
      </div>
    </div>
